...
curl -s https://api.github.com/repos/oscartbeaumont/netlify-dynamic-dns/releases/latest |
    ...
    wget -O nddns -qi - || {
    rm -rf ./nddns
    echo 'Error getting latest binary from Github. Try again or open an issue on the repository!'
    exit 1
}

chmod +x nddns

mv nddns /usr/local/bin/ || {
    rm -rf ./nddns
    echo 'Error moving binary to /usr/local/bin. Maybe try running this script as root!'
    exit 1
}
